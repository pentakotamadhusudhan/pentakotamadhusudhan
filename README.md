![Banner](images/banner.png)
---

# 👋 Hi, I'm Madhu Pentakota  
**Bridging the gap between robust backends and intuitive mobile experiences.**

---

## ✨ About Me  
I’m a **Full-Stack Developer** with over 3 years of experience building things with **Django** and **Flutter**. I enjoy the challenge of connecting heavy-duty backend logic with smooth, user-friendly mobile apps. 

I also have a foundation in **Data Analytics**, which helps me look beyond just the code to see how data can actually improve a product. I’m a big believer in clean code, working closely with my team, and always staying curious about new technologies.

---

## 🛠️ Tech & Tools  

### 📱 Mobile Development (Frontend)  
<div align="center">
  <img src="https://img.shields.io/badge/Flutter-%2302569B.svg?style=for-the-badge&logo=Flutter&logoColor=white" />
  <img src="https://img.shields.io/badge/Dart-%230175C2.svg?style=for-the-badge&logo=dart&logoColor=white" />
  <img src="https://img.shields.io/badge/Material%20Design-757575?style=for-the-badge&logo=material-design&logoColor=white" />
</div>

### 💾 Backend & Databases  
<div align="center">
  <img src="https://img.shields.io/badge/django-%23092e20.svg?style=for-the-badge&logo=django&logoColor=white" />
  <img src="https://img.shields.io/badge/django%20rest-ff1709?style=for-the-badge&logo=django&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/SQLite-07405E?style=for-the-badge&logo=sqlite&logoColor=white" />
  <img src="https://img.shields.io/badge/Firebase-039BE5?style=for-the-badge&logo=Firebase&logoColor=white" />
</div>

### 📊 Data Analytics & Languages
<div align="center">
  <img src="https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" />
 <img src="https://img.shields.io/badge/Matplotlib-%23ffffff.svg?style=for-the-badge&logo=Matplotlib&logoColor=black" />
</div>

---

### ⚙️ Tools & Workflow  
<div align="center">
  <a href="https://developer.apple.com/xcode/">
    <img src="https://img.shields.io/badge/Xcode-1575F9?logo=xcode&logoColor=white" />
  </a>
  <a href="https://developer.apple.com/xcode-cloud/">
    <img src="https://img.shields.io/badge/Xcode%20Cloud-0A84FF?logo=apple&logoColor=white" />
  </a>
  <a href="https://developer.apple.com/testflight/">
    <img src="https://img.shields.io/badge/TestFlight-0A84FF?logo=apple&logoColor=white" />
  </a>
  <a href="https://git-scm.com/">
    <img src="https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white" />
  </a>
  <a href="https://github.com/">
    <img src="https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white" />
  </a>
  
  <a href="https://www.figma.com/">
    <img src="https://img.shields.io/badge/Figma-F24E1E?logo=figma&logoColor=white" />
  </a>

  
  <a href="https://github.com/features/actions">
  <img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white" alt="GitHub Actions" />
</a>
 
</div>

---

## 🚀 Featured Projects

### 🛒 [Local Baba](https://github.com/pentakotamadhusudhan/local_baba)  
*A Hyper-local Marketplace connecting local vendors with customers.*

**Tech Stack:** ![Flutter](https://img.shields.io/badge/Flutter-%2302569B.svg?style=flat&logo=Flutter&logoColor=white) ![Django](https://img.shields.io/badge/django-%23092e20.svg?style=flat&logo=django&logoColor=white) ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)

- 📍 **Mappls Maps:** Integrated for precise hyper-local store tracking and discovery.
- 🛠️ **Admin Dashboard:** Built a custom **Jazzmin** interface for efficient vendor and inventory management.
- 🔍 **Real-time UX:** Implemented fast search and category filtering for seamless shopping.

---

### 🥘 [Indian Sweets Recognition](https://github.com/pentakotamadhusudhan/sweets-ml)  
*Computer Vision project identifying traditional Indian desserts through deep learning.*

**Tech Stack:** ![Python](https://img.shields.io/badge/python-3670A0?style=flat&logo=python&logoColor=ffdd54) ![TensorFlow](https://img.shields.io/badge/TensorFlow-%23FF6F00.svg?style=flat&logo=TensorFlow&logoColor=white) ![PyTorch](https://img.shields.io/badge/PyTorch-%23EE4C2C.svg?style=flat&logo=PyTorch&logoColor=white)

- 🧠 **CNN Model:** Trained an image classification model to recognize 10+ types of Indian sweets.
- 📸 **Dataset:** Curated and pre-processed a custom dataset for high-accuracy recognition.
- ⚡ **Inference:** Optimized for real-time identification using Computer Vision techniques.

[Image of convolutional neural network architecture for image classification]


---

### 🤖 [Rasa AI Bot](https://github.com/pentakotamadhusudhan/rasa-bot)
*Intelligent conversational agent for automated customer interaction.*

**Tech Stack:** ![Rasa](https://img.shields.io/badge/Rasa-5A17EE?style=flat&logo=rasa&logoColor=white) ![Python](https://img.shields.io/badge/python-3776AB?style=flat&logo=python&logoColor=white)

- 💬 **NLP:** Context-aware dialogue management and intent recognition.
- 🔗 **Integration:** Connects with webhooks to fetch real-time data during conversations.

---

### 🏫 [School Management System](https://github.com/pentakotamadhusudhan/school-sys)
*Full-stack management solution for educational institutions.*

**Tech Stack:** ![Flutter](https://img.shields.io/badge/Flutter-%2302569B.svg?style=flat&logo=Flutter&logoColor=white) ![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

- 👥 **Role-Based Access:** Separate interfaces for Students, Teachers, and Administrators.
- 📊 **Management:** Complete system for attendance tracking, grading, and scheduling.

---

### 📊 [Data Analytics Lab](https://github.com/pentakotamadhusudhan/data-analytics)
*Exploratory data analysis and predictive modeling insights.*

**Tech Stack:** ![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white) ![Matplotlib](https://img.shields.io/badge/Matplotlib-%23ffffff.svg?style=flat&logo=Matplotlib&logoColor=black)

- 🔍 **EDA:** In-depth exploratory analysis on retail and marketplace datasets.
- 📈 **Visualization:** Created complex dashboards to identify sales trends and user behavior.
---

### 🧪 Work in Progress  

- **📊 Data Insights Dashboard** _A Django-based internal tool to visualize sales metrics using Matplotlib and Pandas._  

- **🛠️ Service Finder App** _A Flutter application aimed at connecting freelance technicians with local households._  

---

## 🤝 Connect with Me  

<p align="left">
<a href="https://www.linkedin.com/in/m-a-d-h-u/" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="pentakotamadhusudhan" height="30" width="40" /></a>
<a href="mailto:pentakotamadhu74@gmail.com" target="blank"><img align="center" src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" height="30" /></a>
</p>

---

<p align="center">
  <i>"Dream Big, Code Bigger."</i>
</p>
